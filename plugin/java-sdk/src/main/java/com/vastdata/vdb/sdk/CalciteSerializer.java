        ...
        extends VastExpressionSerializer
{
    ...
    CalciteSerializer(EnumeratedSchema schema, String statement)
    {
        ...
        this.config = SqlParser.Config.DEFAULT.withQuotedCasing(
                Casing.UNCHANGED).withUnquotedCasing(Casing.UNCHANGED);
    }

    static String getTableName(String sql)
    {
        try {
            SqlParser.Config config = SqlParser.Config.DEFAULT.withQuotedCasing(
                    Casing.UNCHANGED).withUnquotedCasing(Casing.UNCHANGED);
            SqlSelect node = getSelectNode(
    ...
    private static ImmutableList<SqlNode> collectNodes(SqlKind kind,
    ...
    private static void addNodes(SqlKind kind,
            ImmutableList.Builder<SqlNode> nodeBuilder, SqlNode node)
    {
    ...
    private static void validateCall(List<SqlNode> operands, SqlKind kind)
    {
        ...
        if (operands.size() != expectedOperands) {
            throw new UnsupportedOperationException(
                    "Only binary operators are supported, found " + kind);
        }
        Optional<SqlNode> invalidNode = operands.stream().filter(
                node -> node instanceof SqlBasicCall).filter(
                node -> !SUPPORTED_KINDS.contains(node.getKind())).findFirst();
        if (invalidNode.isPresent()) {
            throw new UnsupportedOperationException(
                    "Unsupported operand " + invalidNode.get().getKind());
        }
    }
    ...
    public int serialize()
    {
        try {
            schema.getSchema().getFields().forEach(field -> {
                this.nameToFieldIndex.put(field.getName(), buildColumn(
                        schema.getBaseFieldIndexByName(field.getName())));
                this.nameToField.put(field.getName(),
                        Field.nullable(null, field.getType()));
            });
            ...
            TraverseResult traverseResult = traverseAndBuild(
                    selectNode.getWhere());
    ...
    Optional<Integer> getLimit()
    {
        try {
            ...
            return Optional.ofNullable(limitLiteral).map(
                    literal -> literal.intValue(true));
        }
        catch (SqlParseException e) {
            throw new RuntimeException(e);
        }
        ...
        try {
            ...
            if (!id.isStar()) {
                return node
                        .getSelectList()
                        .stream()
                        .map(sqlNode -> ((SqlIdentifier) sqlNode).getSimple())
                        .toList();
            }
            return schema
                    .getSchema()
    ...
    private TraverseResult traverseAndBuild(SqlNode node)
    {
        if (node instanceof SqlBasicCall call) {
            ...
            if (kind == SqlKind.IN || kind == SqlKind.NOT_IN) {
                return TraverseResult.wrappedOr(
                        buildInValues(kind, operands.get(0),
            ...
            return switch (kind) {
                case EQUALS -> TraverseResult.notWrapped(
                        buildEqual(left, right));
                case GREATER_THAN -> TraverseResult.notWrapped(
                        buildGreater(left, right, false));
                case GREATER_THAN_OR_EQUAL -> TraverseResult.notWrapped(
                        buildGreater(left, right, true));
                case LESS_THAN -> TraverseResult.notWrapped(
                        buildLess(left, right, false));
                case LESS_THAN_OR_EQUAL -> TraverseResult.notWrapped(
                        buildLess(left, right, true));
        ...
        for (SqlNode node : nodes) {
            if (node instanceof SqlBasicCall call) {
                ...
                if (!columnName.equals(
                        ((SqlIdentifier) ((SqlBasicCall) nodes.get(0))
                                .getOperandList()
                                .get(0)).getSimple())) {
                    throw new UnsupportedOperationException(
                            "All OR predicates must be on the same column");
                }
            }
            else {
                throw new UnsupportedOperationException(
                        "Only basic calls are supported inside OR predicates");
            }
        }
    }

    private int buildInValues(SqlKind kind, SqlNode columnNode,
            SqlNodeList valuesNode)
    {
        int column = traverseAndBuild(columnNode).offset;
        int[] offsets = new int[valuesNode.size()];
        for (int i = 0; i < valuesNode.size(); i++) {
            if (kind == SqlKind.NOT_IN) {
                offsets[i] = buildNotEqual(column,
                        buildValue(columnNode, valuesNode.get(i)));
            }
            else {
                offsets[i] = buildEqual(column,
                        buildValue(columnNode, valuesNode.get(i)));
            }
        }
    ...
    private int buildValue(SqlNode columnNode, SqlNode valueNode)
    {
        ...
        if (valueNode instanceof SqlNumericLiteral numericLiteral) {
            ...
            if (field.getType().getTypeID() == ArrowType.ArrowTypeID.Date) {
                String dateAsString = ((NlsString) Objects.requireNonNull(
                        sqlCharStringLiteral.getValue(), "value")).getValue();
                LocalDate date = LocalDate.parse(dateAsString);
                return buildLiteral(DateLiteral,
            ...
            if (field
                ...
                String timeAsString = ((NlsString) Objects.requireNonNull(
                        sqlCharStringLiteral.getValue(), "value")).getValue();
                long time;
                try {
                    ...
                    throw new UnsupportedOperationException(
                            "Unsupported time format - valid example 2011-12-03T10:15:30");
                }
                return buildLiteral(TimestampLiteral,
                        createTimestampLiteral(builder, time),
                        field.getField(builder));
            }
            else {
                String value = ((NlsString) Objects.requireNonNull(
                        sqlCharStringLiteral.getValue(), "value")).getValue();
                return buildLiteral(StringLiteral, createStringLiteral(builder,
                        builder.createString(value)), field.getField(builder));
            }
        }
        else if (valueNode instanceof SqlBinaryStringLiteral sqlBinaryStringLiteral) {
            return buildLiteral(BinaryLiteral, createBinaryLiteral(builder,
                            builder.createByteVector(requireNonNull(
                                    ((BitString) sqlBinaryStringLiteral.getValue())).getAsByteArray())),
                    field.getField(builder));
        }
        else if (valueNode instanceof SqlLiteral sqlLiteral) {
            ArrowType arrowType = field.getType();
            if (arrowType.getTypeID() == ArrowType.ArrowTypeID.Bool) {
                return buildLiteral(BooleanLiteral,
                        createBooleanLiteral(builder,
                                sqlLiteral.booleanValue()),
                        field.getField(builder));
            }
        }
        throw new UnsupportedOperationException(
                "Unsupported value node type " + valueNode.getClass());
    }

    private int buildSqlNumericalLiteral(SqlNumericLiteral numericLiteral,
            Field field)
    {
        ArrowType arrowType = field.getType();
        switch (arrowType.getTypeID()) {
            case Int: {
                ...
                switch (type.getBitWidth()) {
                    case 8:
                        return buildLiteral(Int8Literal,
                                createInt8Literal(builder,
                                        (byte) numericLiteral.longValue(false)),
                                field.getField(builder));
                    case 16:
                        return buildLiteral(Int16Literal,
                                createInt16Literal(builder,
                                        (short) numericLiteral.longValue(
                                                false)),
                                field.getField(builder));
                    case 32:
                        return buildLiteral(Int32Literal,
                                createInt32Literal(builder,
                                        numericLiteral.intValue(false)),
                                field.getField(builder));
                    case 64:
                        return buildLiteral(Int64Literal,
                                createInt64Literal(builder,
                                        numericLiteral.longValue(false)),
                                field.getField(builder));
                    default:
                        throw new UnsupportedOperationException(
            ...
            case FloatingPoint: {
                return buildLiteral(Float32Literal,
                        createFloat32Literal(builder, intBitsToFloat(
                                toIntExact(numericLiteral.longValue(false)))),
                        field.getField(builder));
            }
            case Decimal: {
                ...
                int valueOffset = org.apache.arrow.computeir.flatbuf.DecimalLiteral.createValueVector(
                        builder, paddedValues);
                return buildLiteral(DecimalLiteral,
                        createDecimalLiteral(builder, valueOffset),
                        field.getField(builder));
            }
            case Date: {
                return buildLiteral(DateLiteral, createDateLiteral(builder,
                                numericLiteral.longValue(false)),
                        field.getField(builder));
            }
        }
        throw new UnsupportedOperationException(
                "Unsupported numeric literal for type " + arrowType);
    }

    private int buildLiteral(Byte literalImpl, int implOffset, int fieldOffset)
    {
        return createExpression(builder, Literal,
                createLiteral(builder, literalImpl, implOffset, fieldOffset));
    }
    ...
    private static class TraverseResult
    {
        ...
        private TraverseResult(int offset, boolean wrappedWithOr,
                boolean wrappedWithAnd)
        {
            this.offset = offset;
