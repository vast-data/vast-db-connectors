...
public class TestVastClient
{
            ...
            throws VastException
    {
        ...
        this.client = new VastClient(httpClient, config,
                new DummyDependenciesFactory(config));

        TransactionManager transactionsManager = new TransactionManager(
                this.client, new VastTransactionFactory());
        SimpleVastTransaction tx = transactionsManager.startTransaction(
                this.endUser);

        if (this.client.schemaExists(tx, schemaName, this.endUser)) {
            Stream<VastObjectDetails> tables = client.listTables(tx, schemaName, 20,
                    this.endUser);
            Iterable<VastObjectDetails> tableIterable = tables::iterator;
            for (VastObjectDetails table : tableIterable) {
                client.dropTable(tx,
                        new DropTableContext(schemaName, table.getName()),
                        this.endUser);
            }
        ...
        this.client.createSchema(tx, schemaName,
                new VastMetadataUtils().getPropertiesString(
                        Collections.emptyMap()), this.endUser);

        transactionsManager.commit(tx, this.endUser);
    }

    private void testInsertAndQueryWideTable(String tableName,
            List<String> sortedBy, TableType tableType)
            throws VastException
    {
        TransactionManager transactionsManager = new TransactionManager(
                this.client, new VastTransactionFactory());
        ...
        for (int i = 0; i < numCols; i++) {
            fields.add(new Field(format("col%d", i),
        ...
        CreateTableContext tableContext = CreateTableContext.create(schemaName,
                tableName, fields, Optional.empty(), properties.build(), null,
                true);
        this.client.createTable(tx, tableContext, this.endUser);
        transactionsManager.commit(tx, this.endUser);

        // 2. Insert Data
        SimpleVastTransaction tx2 = transactionsManager.startTransaction(
                this.endUser);
        int rowCount = 100;
        ...
        try (RootAllocator allocator = new RootAllocator(); VectorSchemaRoot root = VectorSchemaRoot.create(
            ...
            for (int colIdx = 0; colIdx < numCols; colIdx++) {
                IntVector col = (IntVector) root.getVector(
                        format("col%d", colIdx));
            ...
            try (VectorSchemaRoot insertedRowIds = this.client.insertRowsByColumnBatches(
                    tx2, schemaName, tableName, root, this.endpoint,
                    Optional.of(1000), new HashSet<>(sortedBy), tableType, true,
                    new QueryDataExtraParams(), this.endUser)) {
        ...
        // 3. Query and Verify
        SimpleVastTransaction tx3 = transactionsManager.startTransaction(
                this.endUser);
        VastTraceToken token = new VastTraceToken(
                Optional.of(format("%s/%s", schemaName, tableName)),
                tx3.getId(), 3);
        ...
        try (RootAllocator queryAllocator = new RootAllocator()) {
            Supplier<QueryDataResponseHandler> handlerSupplier = () -> {
                ArrowQueryDataSchemaHelper schemaHelper = ArrowQueryDataSchemaHelper.deconstruct(
                        token, arrowSchema, new EmptyVectorAdaptorFactory());
                ...
                QueryDataResponseParser parser = new QueryDataResponseParser(
                        shapingLoggerFactory, token, schemaHelper,
                        VastDebugConfig.DEFAULT, new QueryDataPagination(1),
                        Optional.empty(), queryAllocator, Optional.empty());
                result.set(parser);
            ...
            ProjectionSerializer projections = new ProjectionSerializer(
                    arrowSchema, new EnumeratedSchema(fields));

            client.queryData(tx3, token, schemaName, tableName, arrowSchema,
                    projections, new EmptyPredicateSerializer(),
                    handlerSupplier, new VastSplitContext(0, 1, 1, 1), null,
                    ImmutableList.of(this.endpoint), new VastRetryConfig(3, 3),
                    Optional.empty(), Optional.empty(),
                    new QueryDataPagination(1), false, 0,
                    new QueryDataExtraParams(), this.endUser);

            // 4. Assert results
            QueryDataResponseParser parser = result.get();
            try (VectorSchemaRoot queryResultRoot = parser.next()) {
                assertEquals(queryResultRoot.getRowCount(), rowCount,
                        "Unexpected number of rows returned");
                for (int colIdx = 0; colIdx < numCols; colIdx++) {
                    IntVector col = (IntVector) queryResultRoot.getVector(
                            format("col%d", colIdx));
                    for (int rowIdx = 0; rowIdx < rowCount; rowIdx++) {
                        assertFalse(col.isNull(rowIdx),
                                format("Value at col%d, row%d should not be null",
                                        colIdx, rowIdx));
                        assertEquals(col.get(rowIdx), rowIdx,
                                format("Mismatch at col%d, row%d", colIdx,
                                        rowIdx));
                    }
                }
            }
            ...
            throws VastException
    {
        ...
        testInsertAndQueryWideTable(tableName, Collections.emptyList(),
                TableType.REGULAR);
    }
}
