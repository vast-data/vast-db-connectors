...
public class TestImportDataResultHandler
{
    ...
    public void testSuccessfulRequest(int expectedNumberOfSuccessfulFiles,
            int expectedNumberOfFailedFiles, boolean expectedException)
            throws ImportDataFailure
    {
        when(mockTraceToken.toString()).thenReturn("SomeTraceToken");
        when(mockTrans.getId()).thenReturn(
                Long.parseUnsignedLong("514026084031791104"));
        Multimap<HeaderName, String> m = ImmutableMultimap.of(HeaderName.of(
        ...
        try {
            assertTrue(handleResponse(mockImportDataResponse,
                    new ImportDataResult(expectedNumberOfSuccessfulFiles,
                            expectedNumberOfFailedFiles, Optional.empty()),
                    mockTraceToken));
        }
            ...
            assertEquals(e.getMessage(), String.format(
                    ImportDataFailure.FILES_IMPORT_ERROR_MESSAGE_FORMAT, "",
                    expectedNumberOfSuccessfulFiles,
                    expectedNumberOfFailedFiles));
        }
    }
    ...
    {
        return new Object[][] {new Object[] {2, 0, false},
                new Object[] {1, 1, true},
                new Object[] {0, 2, true}};
    }
    ...
    {
        when(mockTraceToken.toString()).thenReturn("SomeTraceToken");
        when(mockImportDataResponse.getBytes()).thenReturn(
                "test fail message".getBytes(StandardCharsets.UTF_8));
        when(mockImportDataResponse.getStatus()).thenReturn(400);
        when(mockImportDataResponse.getErrorMessage()).thenReturn(
                Optional.empty());
