...
public class EnumeratedSchema
{
    ...
    {
        this(baseFields, Collections.emptyMap());
    }

    public EnumeratedSchema(Collection<Field> baseFields, Map<String, String> metadata)
    {
        this.nodeCount = 0;
        this.nodes = baseFields
                .stream()
                .map(this::visitField)
                .collect(Collectors.toList());
        this.schema = new Schema(baseFields, metadata);
    ...
    private EnumeratedField visitField(Field field)
    {
        ...
        List<EnumeratedField> children = field
                .getChildren()
                .stream()
                .map(this::visitField)
                .collect(Collectors.toList());
    ...
    public void collectProjectionIndices(String baseFieldName,
            List<Integer> projectionPath, Consumer<Integer> resultsConsumer)
    {
    ...
    private static class EnumeratedField
    {
        ...
        public void collectLeafIndices(Consumer<Integer> resultsConsumer)
        {
            if (children.isEmpty()) {
                ...
                children.forEach(
                        child -> child.collectLeafIndices(resultsConsumer));
            }
        }
        ...
        public String toString()
        {
            return "EnumeratedField{" + "index=" + index + ", children=" + children + '}';
        }
    }
}
