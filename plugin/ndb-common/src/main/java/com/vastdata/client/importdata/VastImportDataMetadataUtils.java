...
public final class VastImportDataMetadataUtils
{
    ...
    public static final Field IMPORT_DATA_HIDDEN_FIELD = new Field(
            IMPORT_DATA_HIDDEN_COLUMN_NAME,
    ...
    public static String getTableNameForAPI(String origTableName)
    {
        return isImportDataTableName(origTableName) ?
                origTableName
                .substring(0,
                        origTableName.indexOf(IMPORT_DATA_TABLE_NAME_SUFFIX))
                .trim() :
                origTableName;
    }
    ...
    public static Optional<String> getBigCatalogSearchPath(String schemaName,
            String tableName)
    {
        ...
        if (schemaName.startsWith(
                BIG_CATALOG_SCHEMA_PREFIX) && schemaName.endsWith(
                BIG_CATALOG_SCHEMA_SUFFIX) && tableName.startsWith(
                BIG_CATALOG_TABLE_SEARCH_PATH_PREFIX)) {
            return Optional.of(tableName.substring(
                    BIG_CATALOG_TABLE_SEARCH_PATH_PREFIX.length()));
        }
        else {
    ...
    public static int getImportDataHiddenColumnIndex(List<Field> fields)
    {
        ...
        throw toRuntime(userException(
                format("Could not find %s column in list: %s",
                        IMPORT_DATA_HIDDEN_COLUMN_NAME, fields)));
    }
}
