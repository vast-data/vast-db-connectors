...
public class EvenSizeWithLimitChunkifier
        implements
        BiFunction<ImportDataContext, List<URI>, List<ImportDataContext>>
{
    ...
    private static final Logger LOG = Logger.get(
            EvenSizeWithLimitChunkifier.class);

    @Override
    public List<ImportDataContext> apply(ImportDataContext importDataContext,
            List<URI> endpoints)
    {
        Collection<List<ImportDataFile>> partition = getPartitions(
                importDataContext, endpoints);
        return partition
                .stream()
                .map(filesChunk -> new ImportDataContext(filesChunk,
                        importDataContext.getDest()))
                .collect(Collectors.toList());
    }

    private Collection<List<ImportDataFile>> getPartitions(
            ImportDataContext importDataContext, List<URI> endpoints)
    {
        ...
        if (sourceFiles.size() <= endpoints.size() * limit) {
            Map<Integer, List<ImportDataFile>> chunksCollector = new HashMap<>(
                    endpoints.size());
            for (int i = 0; i < sourceFiles.size(); i++) {
                ImportDataFile file = sourceFiles.get(i);
                chunksCollector
                        .computeIfAbsent(i % endpoints.size(), ArrayList::new)
                        .add(file);
            }
        ...
        LOG.debug("split %d files into %d chunks (using limit=%d)",
                sourceFiles.size(), partition.size(), limit);
        return partition;
    }
}
