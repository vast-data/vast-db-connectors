...
public final class NumOfSplitsEstimator
{
                                               ...
                                               Supplier<Optional<Double>> rowsEstimateSupplier)
    {
        ...
        LOG.info("Estimating number of splits. useMultiplier: %b, maxSplitsSupplier: %d, rowsPerSplitConf: %d, multiplierConf: %d, rowsEstimateSupplier: %s", useMultiplier.getAsBoolean(), maxSplitsSupplier.getAsInt(), rowsPerSplitConf.getAsLong(), multiplierConf.getAsLong(), rowsEstimateSupplier.get().toString());
        ...
        LongSupplier rowPerSplitSupplier =
            () -> {
                if (!useMultiplier.getAsBoolean()) {
                    ...
                    LOG.info("Estimating number of splits, in rowPerSplitSupplier. selectivityEstimation: %f", selectivityEstimation.getAsDouble());
