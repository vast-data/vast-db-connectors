        ...
        implements LeafExecNode
{
    ...
    public Seq<InternalRow> run() {
        ...
        Stream<Attribute> stream = IntStream.range(0, columns.size()).mapToObj(columns::apply).filter(ar -> {
        ...
        List<Attribute> newColumns = scala.collection.JavaConverters.collectionAsScalaIterable(stream.collect(Collectors.toList())).toList();
