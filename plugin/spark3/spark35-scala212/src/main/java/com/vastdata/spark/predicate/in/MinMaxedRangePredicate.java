...
public class MinMaxedRangePredicate
        extends ResultFunction
{
    public static final BiFunction<Predicate, Predicate, Predicate> MIN_MAX_PREDICATE = (min, max) -> new Predicate(
            "AND", new Predicate[] {min, max});
    public static final Function<Expression, Predicate> IS_NULL_PREDICATE = col -> new Predicate(
            "IS_NULL", new Expression[] {col});
    private static final Logger LOG = LoggerFactory.getLogger(
            MinMaxedRangePredicate.class);
    ...
    private static Predicate[] getPredicate(
            ProcessedInValues<LiteralValue<?>> processedInValues,
            Expression[] values, boolean hasNulls)
    {
        Predicate lteMax = new Predicate("<=",
                new Expression[] {values[0], processedInValues.getMax()});
        Predicate gteMin = new Predicate(">=",
                new Expression[] {values[0], processedInValues.getMin()});
            ...
            Expression[] values)
    {
        ...
        if (processedInValues.isFullRange()) {
            ...
            return getPredicate(processedInValues, values,
                    processedInValues.getNullCount() > 0);
        }
        if (!fullRangeOnly && nonNullValuesCount > threshold) {
            ...
            LOG.info(
                    "Skipping optimization for range: {}, values length: {}, threshold: {}",
                    processedInValues, values.length - 1, threshold);
