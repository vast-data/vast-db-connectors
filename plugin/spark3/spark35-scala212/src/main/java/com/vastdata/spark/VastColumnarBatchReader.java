        ...
        implements PartitionReader<ColumnarBatch>
{
            ...
            VastSchedulingInfo schedulingInfo, boolean forAlter)
    {
        ...
        List<Field> fields = TypeUtil.sparkSchemaToArrowFieldsList(schema);
        List<Field> collect;
        if (this.forAlter) {
            collect = fields.stream().map(f -> f.getName().equals(VASTDB_SPARK_ROW_ID_NONNULL.getName()) ? ROW_ID_FIELD : f).collect(Collectors.toList());
        }
        else {
            collect = fields.stream().filter(f -> !f.getName().equals(VASTDB_SPARK_ROW_ID_NONNULL.getName())).collect(Collectors.toList());
        }
        this.projectionSchema = new Schema(collect);
        ...
        try {
            while (true) {
                if (nonNull(parser) && parser.hasNext()) {
                    ...
                    Function<FieldVector, ColumnVector> mapper = new FieldToColumnMapper<>(allocator, ArrowColumnVector::new);
