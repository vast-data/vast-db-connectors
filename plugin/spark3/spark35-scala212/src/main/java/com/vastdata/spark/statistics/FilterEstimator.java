...
public final class FilterEstimator
{
    ...
    // Scala 2.12 doesn't have scala.jdk.javaapi.ScalaConverters.toScala, so we backport it here
    private static <T> scala.Option<T> toScala(Optional<T> o)
    {
        return o.isPresent() ?
                Option$.MODULE$.apply(o.get()) :
                Option$.MODULE$.empty();
    }
            ...
            ColumnStatistics colStats, StructField field, Predicate predicate)
    {
        final ValueInterval statsInterval = ValueInterval.apply(
                toScala(colStats.min()), toScala(colStats.max()),
                field.dataType());
            ...
            boolean updateStatistics, String traceToken)
    {
        ...
        switch (predicate.name()) {
            ...
            case "=": {
                ...
                if (isStringOrBinaryOrWithinRange(colStats, field, predicate)) {
                    if (updateStatistics && "=".equals(predicate.name())) {
                        ...
                        if ((dt instanceof StringType) || (dt instanceof BinaryType)) {
                            ...
                            LOG.info(
                                    "Estimating op selectivity, new stats. pred: {}, newStat",
                                    predicate.toString(), newStat.toString());
                        ...
                        LOG.info(
                            "Estimating op selectivity, integrating distinct count. pred: {}, distinct count: {}",
                            predicate.toString(),
                ...
                final double selectivity = "=".equals(predicate.name()) ?
                        percent :
                        (1.0 - percent);
                LOG.info("Estimating op selectivity. pred: {}, selectivity: {}",
                        predicate.toString(), selectivity);
                return selectivity;
            }
            case ">":
            case "<":
            ...
            case ">=": {
                ...
                org.apache.spark.sql.connector.expressions.Literal<?> literal = (org.apache.spark.sql.connector.expressions.Literal<?>) predicate.children()[1];
                double max = EstimationUtils.toDouble(colStats.max().get(),
                        literal.dataType());
                double min = EstimationUtils.toDouble(colStats.min().get(),
                        literal.dataType());
                long ndv = colStats.distinctCount().getAsLong();
                double numericLiteral = EstimationUtils.toDouble(
                ...
                LOG.info(
                        "Estimating predicate({}): {}: name: {} min: {} max: {} selectivity: {} ndv: {}",
            ...
            List<List<VastPredicate>> predicates, Statistics statistics, String traceToken)
    {
        ...
        LOG.info("Estimating selectivity for predicates: {}",
                predicates.toString());
