...
public class NDBResolutionRule
        extends org.apache.spark.sql.catalyst.rules.Rule<org.apache.spark.sql.catalyst.plans.logical.LogicalPlan>
{
    private static final Logger LOG = LoggerFactory.getLogger(NDBResolutionRule.class);
    ...
    public LogicalPlan apply(LogicalPlan plan)
    {
        if (plan instanceof UpdateTable) {
            ...
            if (u.resolved() && u.rewritable() && !u.aligned()) {
                ...
                LogicalPlan transformedTable = u.table().transform(PartialFunction$.MODULE$.apply(func));
                scala.collection.Seq<Assignment> newAssignments = AssignmentUtils.alignUpdateAssignments(transformedTable.output(), u.assignments());
            ...
            Function1<LogicalPlan, LogicalPlan> func = lp -> {
                if (lp instanceof DataSourceV2Relation) {
                    ...
                    AttributeReference rowIdAttRef =
                            new AttributeReference(SPARK_ROW_ID_FIELD.name(), SPARK_ROW_ID_FIELD.dataType(), false,
                                    Metadata.empty(), ExprId.apply(0), List.empty());
            ...
            scala.collection.Seq<FieldName> columns = drop.columnsToDrop();
            ...
            scala.collection.Seq<QualifiedColType> columns = drop.columnsToAdd();
            IntStream.range(0, columns.size()).forEach(i -> {
            ...
            scala.collection.Seq<QualifiedColType> colsToAdd = replaceColumns.columnsToAdd();
