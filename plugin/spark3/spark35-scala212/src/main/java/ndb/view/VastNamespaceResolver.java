        ...
        implements BiFunction<Seq<String>, String[], String[]>
{
    ...
    public String[] apply(Seq<String> fullName, String[] currentNamespace)
    {
        ...
        LOG.debug(
                "Trying to resolve namespace. fullName: {}, currentNamespace: {}",
                fullName, Arrays.toString(currentNamespace));
        if (uRelNameLength == 1) {
            return currentNamespace;
        }
        else {
            final int startIndex = indexOfStripDefaultCatalogFromFullName(
                    fullName);
            ...
            LOG.debug("Resolved namespace: {}",
                    Arrays.toString(namespaceForLookup));
