        ...
        implements LeafExecNode
{
    ...
    private DropNDBViewCommand(final ResolvedIdentifier resolvedIdentifier,
            final boolean ifExists, final DropNDBViewPlan original)
    {
        super();
    ...
    public static DropNDBViewCommand instance(final DropNDBViewPlan plan)
    {
        ...
        if (child instanceof ResolvedIdentifier) {
            return new DropNDBViewCommand((ResolvedIdentifier) child,
                    plan.ifExists, plan);
        }
    ...
    public Seq<InternalRow> run()
    {
        ...
        final boolean dropped = catalog.dropView(
                resolvedIdentifier.identifier(), Optional.empty());
        if (!ifExists && !dropped) {
            throw new RuntimeException(
                    new NoSuchViewException(resolvedIdentifier.identifier()));
        }
        final InternalRow row = new GenericInternalRow(1);
        row.update(0, dropped);
        final Seq<InternalRow> result = JavaConverters
                .asScalaIteratorConverter(
                        Arrays.stream(new InternalRow[] {row}).iterator())
                .asScala()
                .toSeq();
