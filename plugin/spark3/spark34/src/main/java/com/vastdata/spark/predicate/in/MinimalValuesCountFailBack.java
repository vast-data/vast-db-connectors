...
public class MinimalValuesCountFailBack
        extends ExplicitValuesPredicateList
{
    private static final Logger LOG = LoggerFactory.getLogger(
            MinimalValuesCountFailBack.class);
    ...
    @Override
    public Predicate[] apply(ProcessedInValues processedInValues,
            Expression[] values)
    {
        if (values.length > minValuesThreshold) {
            ...
            LOG.info(
                    "Value count: {} is below optimization min values threshold: {}",
                    values.length - 1, minValuesThreshold);
