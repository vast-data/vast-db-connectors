        ...
        implements PartitionReaderFactory
{
    ...
    private VastSchedulingInfo getSchedInfo(SimpleVastTransaction tx, String schemaName, String tableName)
    {
        ...
        try (VastAutocommitTransaction schedTx = VastAutocommitTransaction.wrap(Optional.ofNullable(tx), vastClient, () -> transactionsManager.startTransaction(new StartTransactionContext(false, true)))) {
            VastTraceToken traceToken = schedTx.generateTraceToken(Optional.empty());
            return vastClient.getSchedulingInfo(schedTx, traceToken, schemaName, tableName);
        }
    }
