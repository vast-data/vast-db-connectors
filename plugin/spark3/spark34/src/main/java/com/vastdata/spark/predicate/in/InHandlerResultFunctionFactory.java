...
public final class InHandlerResultFunctionFactory
{
    ...
    public static ResultFunction get(Map<String, Object> parsingRules,
            DataType dataType, ResultFunctionMethod... methods)
    {
        ResultFunction chain = null;
        for (ResultFunctionMethod method : methods) {
            ResultFunction next = getResultFunction(parsingRules, dataType,
                    method);
            if (chain != null) {
                next.setNext(chain);
            }
    ...
    private static ResultFunction getResultFunction(
            Map<String, Object> parsingRules, DataType dataType,
            ResultFunctionMethod method)
    {
        switch (method) {
            case EXPLICIT_VALUES:
                return new ExplicitValuesPredicateList();
            case MIN_MAX:
                int threshold = (int) parsingRules.getOrDefault(
            ...
            case MINIMAL_VALUE_COUNT_CHECK:
                int minValuesThreshold = (Integer) parsingRules.getOrDefault(
            ...
            default:
                throw new IllegalArgumentException(
                        format("Unknown IN predicate handling method: %s",
                                method));
        }
    }
}
