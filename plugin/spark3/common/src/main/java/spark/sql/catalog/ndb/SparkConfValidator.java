...
public class SparkConfValidator
{
    ...
    public final static String SETTING_DISABLE_SPARK_DUPLICATE_WRITES_PROTECTION = "spark.ndb.dangerous_disable_spark_duplicate_writes_protection";
    final static String FORMAT_UNSAFE_SPARK_CONFIGURATION = String.format(
            "Unsafe Spark configuration blocks write.\nExpected:\n\t%s == 1\n\t%s == false\nGot:\n\t%s == %%d\n\t%s == %%b",
            SETTING_SPARK_MAX_FAILURES, SETTING_SPARK_SPECULATION,
            SETTING_SPARK_MAX_FAILURES, SETTING_SPARK_SPECULATION);
    private static final Logger LOG = LoggerFactory.getLogger(
            SparkConfValidator.class);
    ...
    public SparkConfValidator(
            BiFunction<String, Integer, Integer> intConfSupplier,
            BiFunction<String, Boolean, Boolean> boolConfSupplier)
    {
        sparkTaskMaxFailures = intConfSupplier.apply(SETTING_SPARK_MAX_FAILURES,
                4);
        LOG.debug("{} = {}", SETTING_SPARK_MAX_FAILURES, sparkTaskMaxFailures);
        sparkSpeculation = boolConfSupplier.apply(SETTING_SPARK_SPECULATION,
                false);
        LOG.debug("{} = {}", SETTING_SPARK_SPECULATION, sparkSpeculation);
        disableSparkDuplicateWritesProtection = boolConfSupplier.apply(
                SETTING_DISABLE_SPARK_DUPLICATE_WRITES_PROTECTION, false);
        LOG.debug("{} = {}", SETTING_DISABLE_SPARK_DUPLICATE_WRITES_PROTECTION,
        ...
        writeError = disableSparkDuplicateWritesProtection || ((1 == sparkTaskMaxFailures) && !sparkSpeculation) ?
                ...
                Optional.of(new RuntimeException(
                        String.format(FORMAT_UNSAFE_SPARK_CONFIGURATION,
                                sparkTaskMaxFailures, sparkSpeculation)));
