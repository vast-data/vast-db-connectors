...
{
    private static final Logger LOG = LoggerFactory.getLogger(SparkVectorAdaptorUtil.class);
    ...
    public static void convertNonMicroTSVectorToMicroTSVector(TimeStampVector src, TimeStampMicroTZVector dst)
    {
        ...
        LOG.debug("Converting {} to {} with value count: {}", src.getClass().getName(), dst.getClass().getName(), valueCount);
    ...
    public static boolean requiresTSConversion(TimeStampVector fieldVector)
    {
        return !(fieldVector instanceof TimeStampMicroTZVector);
    }
}
