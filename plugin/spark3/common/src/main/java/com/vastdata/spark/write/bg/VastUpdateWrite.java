...
public class VastUpdateWrite
        implements VastWriteStrategy
{
    private static final Logger LOG = LoggerFactory.getLogger(VastUpdateWrite.class);

    private final WriteContext ctx;

    public VastUpdateWrite(
            ...
            throws VastException
    {
        ...
        LOG.debug("{} Updating next chunk of {} rows, {} columns, hash={}, schema: {}",
        ...
        try (nextChunk) {
            ctx.getVastClientSupplier()
                    ...
                    .updateRows(ctx.getTx(), ctx.getSchemaName(), ctx.getTableName(), nextChunk, ctx.getEndpoint(),
