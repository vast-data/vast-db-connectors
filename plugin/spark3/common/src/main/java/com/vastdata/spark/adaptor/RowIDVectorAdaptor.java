        ...
        implements VectorAdaptor
{
            ...
            BufferAllocator allocator)
    {
        ...
        try {
            if (field.equals(ROW_ID_INT64_FIELD)) {
                return copyVectorBuffers(vector,
                        ROW_ID_INT64_FIELD.createVector(allocator));
            }
            else {
                return copyVectorBuffers(vector,
                        ROW_ID_DEC128_FIELD.createVector(allocator));
            }
        }
        finally {
            vector.close();
