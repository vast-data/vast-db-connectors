...
public final class SparkVectorAdaptorFactory
        implements VectorAdaptorFactory
{
    ...
    {
        if (field.equals(ROW_ID_UINT64_FIELD) || field.equals(
                ROW_ID_DEC128_FIELD)) {
            return Optional.of(new RowIDVectorAdaptor());
        }
        else if (field.getType().getTypeID().equals(
                ArrowType.ArrowTypeID.FixedSizeBinary)) {
            return Optional.of(new CharNVectorAdaptor());
        }
        else if (field.getType().getTypeID().equals(
                ArrowType.ArrowTypeID.Timestamp) && requiresTSConversion(
                field)) {
