...
public final class SparkPersistentStatistics implements VastStatisticsStorage<Table, Statistics>
{
    private static final Logger LOG = LoggerFactory.getLogger(SparkPersistentStatistics.class);

    private final VastClient client;
    private final StatisticsUrlExtractor<Table> statisticsUrlHelper = NDB.getSparkDependenciesFactory().getStatisticsUrlHelper();
    private final String tag = NDB.getSparkDependenciesFactory().getConnectorVersionedStatisticsTag();

    private ObjectMapper getMapper(Table table)
    {
        return SparkStatisticsMapper.instance(table);
    }

    private final LoadingCache<Table, Optional<Statistics>> cache;
    ...
    @NotNull
    private CacheLoader<Table, Optional<Statistics>> getCacheLoader(VastClient client)
    {
        return new CacheLoader<Table, Optional<Statistics>>()
        {
            @Override
            public Optional<Statistics> load(@NotNull Table table)
            {
                ...
                if (tsBuffer.isPresent()) {
                    try {
                        ...
                        LOG.info("Fetched {} statistics file from S3 bucket {}: {}", keyName, bucketName, ts);
    ...
    public Optional<Statistics> getTableStatistics(Table table) {
        try {
            ...
            Optional<Statistics> result = cache.get(table);
            if (result.isPresent()) {
            ...
            LOG.warn("Failed to fetch statistics file for table {} from cache\n", table.name());
            return Optional.empty();
        }
    }
    ...
    public void setTableStatistics(Table table, Statistics tableStatistics) {
        ...
        try {
            cache.get(table, () -> Optional.of(tableStatistics));
        } catch (ExecutionException e) {
