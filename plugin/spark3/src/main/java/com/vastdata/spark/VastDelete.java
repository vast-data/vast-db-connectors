        ...
        implements SupportsDeleteV2
{
    ...
    public VastDelete(VastTable table, Supplier<VastClient> clientSupplier)
    {
        ...
        this.table = table;
        this.parsePredicates = predicates -> VastPredicatePushdown.parse(predicates, table.getTableMD().schema);
    }
    ...
    private EnumeratedSchema extractSchema()
    {
        if (enumeratedSchema == null) {
            enumeratedSchema = new EnumeratedSchema(TypeUtil.sparkSchemaToArrowFieldsList(table.getTableMD().schema));
        }
    ...
    {
        VastScanBuilder vastScanBuilder = new VastScanBuilder(this.table, getOptionMap()); // we don't support post-filtering the output of this scan
