...
public final class InHandler
{
    private static final InValuesProcessor<Integer> exceptionThrower = new InValuesProcessor<Integer>() {
        ...
        @Override
        public boolean isFullRange(Integer max, Integer min, int nullCount, Expression[] values)
        {
            throw new RuntimeException("Unsupported type");
        }

        @Override
        public ProcessedInValues getProcessedInValues(Expression[] values, DataType dataType)
        {
            return null;
        }
    };
            ...
            implements InValuesProcessor<Integer>
    {
        ...
        public boolean isFullRange(Integer max, Integer min, int nullCount, Expression[] values)
        {
            if (max == null || min == null) {
                ...
                return nonNullValuesCount - range == 1;
            }
        }
    }
            ...
            implements InValuesProcessor<Long>
    {
        ...
        @Override
        public boolean isFullRange(Long max, Long min, int nullCount, Expression[] values)
        {
            if (max == null || min == null) {
                throw new IllegalArgumentException(format("Min or max values must not be null: %s, %s", max, min));
            }
