...
{
    private static void assertFilterCompactionNegative(StructType schema,
                                                       Predicate[] expected,
                                                       Object[] vals)
    {
        Predicate[] predicates = {new In("x", vals).toV2()};
        VastPredicatePushdown pushdown = VastPredicatePushdown.parse(predicates,
                schema);
        assertTrue(pushdown.getPostFilter().isEmpty());
        java.util.List<VastPredicate> predicateList = Iterables.getOnlyElement(
                pushdown.getPushedDown());
        assertEquals(predicateList.size(), vals.length);
        Predicate[] actual = predicateList
                .stream()
    ...
    private static void assertFilterCompactionPositive(StructType schema,
                                                       java.util.List<Predicate> expected,
                                                       Integer[] vals)
    {
        Predicate[] predicates = {new In("x", vals).toV2()};
        VastPredicatePushdown pushdown = VastPredicatePushdown.parse(predicates,
                schema);
        assertTrue(pushdown.getPostFilter().isEmpty());
        java.util.List<Predicate> minMaxPredicates = pushdown
                .getPushedDown()
                .stream()
    ...
    public void testSanity()
    {
        ...
        Predicate[] predicates = {buildBinaryPred("=", buildFieldRef("i"),
                buildLit(5, DataTypes.IntegerType))};
        VastPredicatePushdown pushdown = VastPredicatePushdown.parse(predicates,
                schema);
        System.out.println("sanity-pushdown: " + pushdown
                .getPushedDown()
                .get(0)
                .get(0)
                .getPredicate());
    ...
    public void testOrExpression()
    {
        StructType schema = new StructType(
                new StructField[] {createStructField("i", DataTypes.IntegerType,
                        true)});
        Predicate[] predicates = {buildBinaryPred("OR",
    ...
    public void testBetween()
    {
        StructType schema = new StructType(
                new StructField[] {createStructField("i", DataTypes.IntegerType,
                        true)});
        Predicate[] predicates = {buildBinaryPred("AND",
                buildBinaryPred(">", buildFieldRef("i"),
    ...
    public void testNotEqual()
    {
        StructType schema = new StructType(
                new StructField[] {createStructField("i", DataTypes.IntegerType,
                        true)});
        Predicate[] predicates = {buildUnaryPred("IS_NOT_NULL",
                buildFieldRef("i")),
                buildUnaryPred("NOT", buildBinaryPred("=", buildFieldRef("i"),
                        buildLit(5, DataTypes.IntegerType)))};
        VastPredicatePushdown pushdown = VastPredicatePushdown.parse(predicates,
                schema);
    ...
    {
        StructType schemaInt = new StructType(
                new StructField[] {createStructField("x", DataTypes.IntegerType,
                        true)});
        ...
        Predicate lteMax = new Predicate("<=", new Expression[] {refInt,
                new LiteralValue<>(maxNotInclusive - 1,
                        DataTypes.IntegerType)});
        Predicate gteMin = new Predicate(">=", new Expression[] {refInt,
                new LiteralValue<>(minInclusive, DataTypes.IntegerType)});
        java.util.List<Predicate> expectedCompacted = Collections.singletonList(
                MIN_MAX_PREDICATE.apply(gteMin, lteMax));
        Integer[] vals = IntStream
                .range(minInclusive, maxNotInclusive)
                .boxed()
                .toArray(Integer[]::new);
        assertFilterCompactionPositive(schemaInt, expectedCompacted, vals);

        // test graceful compaction
        expectedCompacted = Arrays.asList(
                MIN_MAX_PREDICATE.apply(gteMin, lteMax),
                IS_NULL_PREDICATE.apply(refInt));
        Integer[] valsWithNull = new Integer[vals.length + 1];
        valsWithNull[0] = null;
        System.arraycopy(vals, 0, valsWithNull, 1, vals.length);
        assertFilterCompactionPositive(schemaInt, expectedCompacted,
                valsWithNull);
        ...
        vals = IntStream
                ...
                .boxed()
                .toArray(Integer[]::new);
        Predicate[] expectedFull = IntStream
                .range(minInclusive, maxNotInclusive)
                .filter(i -> i % 2 == 0)
                .mapToObj(i -> new Predicate("=", new Expression[] {refInt,
                        new LiteralValue<>(i, DataTypes.IntegerType)}))
                .toArray(Predicate[]::new);
        ...
        StructType schemaString = new StructType(
                new StructField[] {createStructField("x", DataTypes.StringType,
                        true)});
        NamedReference refString = FieldReference.column("x");
        ImmutableList<String> strings = ImmutableList.of("a", "b", "c", "d",
                "e");
        String[] stringVals = strings.toArray(new String[0]);
        expectedFull = strings
                .stream()
                .map(s -> new Predicate("=", new Expression[] {refString,
                        new LiteralValue<>(s, DataTypes.StringType)}))
                .toArray(Predicate[]::new);
