...
import static com.vastdata.mockserver.handle.MockSchemaUtil.deserializeCreateViewRequestBody;
...
public class TestSparkViewMetadata
{
            ...
            throws IOException
    {
        ...
        StructType testSchema = new StructType(
                new StructField[] {new StructField("b", DataTypes.BooleanType,
                        true, Metadata.empty())});
        SparkViewMetadata unit = new SparkViewMetadata(ident, false, true,
                Option.empty(), cols, props, "select a from ndb.buc.schem.tab",
                testSchema, "ndb", namespace);
        ...
        try (RootAllocator allocator = new RootAllocator()) {
            byte[] serializedSchema = VastArrowPayloadSerializer
                    .getInstanceForSchema()
                    .apply(schema, (BufferAllocator) allocator)
                    .get();
            byte[] serializedViewDetails = VastPayloadSerializer
            ...
            byte[] bytes = util.serializeCreateViewRequestBody(serializedSchema,
                    serializedViewDetails);
            System.out.printf(
                    "serializedSchema = %s, serializedViewDetails = %s, bothLengths = %s, bytes = %s%n",
                    serializedSchema.length, serializedViewDetails.length,
                    bothLengths, bytes.length);

            try (VectorSchemaRoot vectorSchemaRoot = deserializeCreateViewRequestBody(
                    bytes).getViewDetails()) {
