...
public final class SparkPlannerUtil
{
    ...
    public static final Seq<SparkPlan> EMPTY_RESULT_SEQ = (Seq<SparkPlan>) Seq$.MODULE$.empty();
            ...
            Seq<String> multipartIdentifier)
    {
        ...
        for (; i < multipartIdentifier.size() - 1; i++) {
            buf.addOne(multipartIdentifier.apply(i));
        }
        buf.addOne(multipartIdentifier.apply(i) + VAST_THROW_RCLS_ERROR);
        return buf.toSeq();
    }
            ...
            Seq<String> multipartIdentifier)
    {
        ...
        if (indexOfRclsSuffix > 0) {
            buf.addOne(name.substring(0, indexOfRclsSuffix));
        }
        else {
            buf.addOne(name);
        }
            ...
            Seq<String> tableNameSeq)
    {
        ...
        if (isForRowLevelOp(tableName)) {
            ...
            for (int i = 0; i < tableNameSeq.size() - 1; i++) {
                nameSeqWithoutRCLSSuffix.addOne(tableNameSeq.apply(i));
            }
            nameSeqWithoutRCLSSuffix.addOne(
