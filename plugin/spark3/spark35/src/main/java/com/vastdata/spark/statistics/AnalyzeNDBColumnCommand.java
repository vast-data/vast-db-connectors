        ...
        implements LeafExecNode
{
    ...
    public Seq<InternalRow> run()
    {
        ...
        IntStream.range(0, columns.size()).mapToObj(columns::apply).filter(
                ar -> {
                    ...
                    ar.references().foreach(ref -> {
                        if (ref.name().equals(
                                SPARK_INT64_ROW_ID_FIELD.name()) || ref
                                .name()
                                .equals(SPARK_DEC128_ROW_ID_FIELD)) {
                            isRowId.set(true);
                        }
                        return null;
                    });
        ...
        Tuple2<Object, Map<Attribute, ColumnStat>> calculatedStats = CommandUtils$.MODULE$.computeColumnStats(
                session, rel, newColumns);
        ...
        AttributeMap<ColumnStat> columnStats = getColumnStatAttributeMap(
                calculatedStats._2);
        Statistics stats = new Statistics(sizeInBytes,
        ...
        if (LOG.isDebugEnabled()) {
            stats.attributeStats().foreach(tup -> {
                ...
                if (maxValue == null) {
                    ...
                    LOG.debug("max = {} of class: {}", maxValue,
                            maxValue.getClass());
                }
                ...
                if (minValue == null) {
                    ...
                    LOG.debug("min = {} of class: {}", minValue,
                            minValue.getClass());
                }
                return null;
            });
