...
public final class SparkPersistentStatistics
        implements VastStatisticsStorage<VastTable, Statistics>
{
    private static final Logger LOG = LoggerFactory.getLogger(
            SparkPersistentStatistics.class);

    private final VastClient client;
    private final StatisticsUrlExtractor<Table> statisticsUrlHelper = (StatisticsUrlExtractor<Table>) NDB
            .getSparkDependenciesFactory()
            .getStatisticsUrlHelper();
    private final String tag = NDB
            .getSparkDependenciesFactory()
            .getConnectorVersionedStatisticsTag();
    ...
    public SparkPersistentStatistics(VastClient client, VastConfig config)
    {
        ...
        this.cache = CacheBuilder.newBuilder().maximumSize(
                config.getMaxStatisticsFilesSupportedPerSession()).build(
                getCacheLoader(client));
    }
    ...
    private CacheLoader<CacheKey, Optional<Statistics>> getCacheLoader(
            VastClient client)
    {
        return new CacheLoader<CacheKey, Optional<Statistics>>()
        {
            @Override
            public Optional<Statistics> load(CacheKey tableCacheKey)
            {
                VastTable table = tableCacheKey.getTable();
                StatisticsUrl statisticsUrl = StatisticsUrl.extract(table,
                        statisticsUrlHelper, tag);
                ...
                Optional<String> tsBuffer = client.s3GetObj(keyName,
                        bucketName);
                LOG.debug("Fetched object {} from Vast", keyName);
                if ((tsBuffer != null) && (tsBuffer.isPresent())) {
                    try {
                        Statistics ts = getMapper(table).readValue(
                                tsBuffer.get(), Statistics.class);
                        LOG.debug(
                                "Fetched {} statistics file from S3 bucket {}: {}",
                                keyName, bucketName, ts);
                        return Optional.of(ts);
                    }
                    catch (Exception e) {
                        LOG.error(
                                format("Failed to parse table statistics file, perhaps table wasn't analyzed or corrupted statistics file: %s",
                                        tsBuffer.get()), e);
                        return Optional.empty();
                    }
                }
                ...
                // We do not want it to persist since no explicit analyze was called by the user
                {
                    ...
                    try {
                        ...
                        VastStatistics tableStats = StatsUtils.getTableLevelStats(
                                client, schemaPath, parsedURL.getTableName());
                        //populate the cache
                        org.apache.spark.sql.catalyst.plans.logical.Statistics newStats = vastTableStatsToCatalystStatistics(
                                tableStats);
                        LOG.debug(
                                "Converted statistics for table {} to spark persistent statistics: {}",
                                table.name(), newStats.simpleString());
                        return Optional.of(newStats);
                    }
                    catch (Exception e) {
                        LOG.warn("Failed fetching table level statistics", e);
                        return Optional.empty();
                    }
                }
            }
    ...
    public Optional<Statistics> getTableStatistics(VastTable table)
    {
        ...
        try {
            LOG.debug("Fetching statistics file for table {} from cache",
                    table.name());
            Optional<Statistics> result = cache.get(tableCacheKey);
            if (result.isPresent()) {
                LOG.debug("Successfully fetched statistics file for table {}",
                        table.name());
            }
            else {
                LOG.debug("Could not fetch statistics file for table {}",
                        table.name());
            }
            return result;
        }
            ...
            LOG.warn(
                    format("Failed to fetch statistics file for table %s from cache",
                            table.name()), e);
            return Optional.empty();
        }
    }
    ...
    public void setTableStatistics(VastTable table, Statistics tableStatistics)
    {
        ...
        if (!bucketName.equals(BIG_CATALOG_BUCKET_NAME)) {
            ...
            LOG.debug("Uploading statistics file {} to S3 bucket {}", keyName,
                    bucketName);
            try {
                String tableStatisticsStr = getMapper(table).writeValueAsString(
                        tableStatistics);
                LOG.debug(
                        "Storing table statistics: file name: {}, contents: {}, bucket: {}",
                        keyName, tableStatisticsStr, bucketName);
                client.s3PutObj(keyName, bucketName, tableStatisticsStr);
            }
            catch (Exception e) {
                LOG.warn(
                        "Failed to upload table statistics file to S3 bucket {}",
                        bucketName);
    ...
    public static class CacheKey
    {
        ...
        public boolean equals(Object obj)
        {
            ...
            return Objects.equals(this.getTable().getTableHandleID(),
                    other.getTable().getTableHandleID());
        }
