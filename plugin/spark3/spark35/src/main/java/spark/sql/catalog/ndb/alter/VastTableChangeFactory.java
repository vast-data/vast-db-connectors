...
import static com.vastdata.spark.SparkArrowVectorUtil.VASTDB_SPARK_INT64_ROW_ID_COLUMN_NAME;
...
public class VastTableChangeFactory
{
    ...
    private static final BiFunction<List<Field>, String[], List<Field>> filterExistingColumns = (existingColumns, testColumnNames) -> {
        ...
        return existingColumns.stream().filter(
                f -> testColumnNamesSet.contains(f.getName())).collect(
                Collectors.toList());
    };
    ...
    public VastTableChangeFactory(VastConfig vastConfig, String schemaName,
            String tableName)
    {
        this.vastConfig = vastConfig;
        this.schemaName = schemaName;
        this.tableName = tableName;
        this.streamFieldsAsDropColumnContext = fields -> fields.stream().map(
                field -> new TableColumnLifecycleContext(schemaName, tableName,
                        field));
    }

    private String deleteActionToStr(TableChange.DeleteColumn deleteColumn)
    {
        return format("DeleteColumn(name:%s,ifExists:%s)",
                Arrays.toString(deleteColumn.fieldNames()),
                deleteColumn.ifExists());
    }
        ...
        for (TableChange tableChange : tableChanges) {
            if (tableChange instanceof TableChange.DeleteColumn) {
                ...
                if (shouldSkipDelete(delete)) {
                    LOG.debug(
                            "Skipping drop column action for table change: {}",
                            deleteActionToStr(delete));
                }
                else {
                    LOG.debug("Adding drop column action for table change: {}",
                            deleteActionToStr(delete));
                    builder.add(getDeleteAction(columnFetcher, delete));
                }
            }
            else if (tableChange instanceof TableChange.AddColumn) {
                LOG.debug("Adding add column action for table change: {}",
                        tableChange);
                builder.add(getAddAction((TableChange.AddColumn) tableChange));
            }
            else if (tableChange instanceof TableChange.RenameColumn) {
                LOG.debug("Adding rename column action for table change: {}",
                        tableChange);
                builder.add(getRenameAction(
                        (TableChange.RenameColumn) tableChange));
            }
            else if (tableChange instanceof TableChange.SetProperty) {
                if (SORTED_BY_PROPERTY.equals(
                        ((TableChange.SetProperty) tableChange).property())) {
                    LOG.debug("Updating sorted_by property: {}", tableChange);
                    builder.add(getSortedByAction(
                            ((TableChange.SetProperty) tableChange).value()));
                }
            }
            else {
                throw toRuntime(new VastUserException(
                        format("Unsupported table change: %s", tableChange)));
            }
        }
        return new CompositeTableChange(builder.build());
    }

    private VastTableChange getSortedByAction(String value)
    {
        return (client, tx) -> {
            try {
                TableLayout layout = client.fetchTableLayout(tx, schemaName,
                        tableName, PAGE_SIZE, new QueryDataExtraParams(), null);

                if (layout.hasSortedColumns()) {
                    throw new VastUserException(
                            "Sorted can be applied only to a non sorted table");
                }
                List<String> requestedSortColumns = getSortedByColumns(value);
                List<String> availableColumnNames = layout
                        .getSchema()
                        .getFields()
                        .stream()
                        .map(Field::getName)
                ...
                properties.put(SORTED_BY_PROPERTY,
                        Optional.of(requestedSortColumns));
                LOG.info("altering table: schema:{}, table:{}", schemaName,
                        tableName);
                client.alterTable(tx, schemaName, tableName,
                        AlterTableContext.create(null, properties,
                                availableColumnNames, true), null);
            }
            ...
            throws VastUserException
    {
        ...
        VerifyParam.verify(fieldNames.length == 1,
                format("Received multiple column names to delete: %s",
                        Arrays.toString(fieldNames)));
        String name = fieldNames[0];
        return VASTDB_SPARK_INT64_ROW_ID_COLUMN_NAME.equals(
                name) || ROW_ID_FIELD_NAME.equals(name);
    }
            ...
            throws VastUserException
    {
        ...
        VerifyParam.verify(oldNames.length == 1,
                "Received multiple column names to rename");
        String oldName = oldNames[0];
        return (client, tx) -> {
            try {
                client.alterColumn(tx, schemaName, tableName,
                        new AlterColumnContext(oldName, rename.newName(),
    ...
    private VastTableChange getDeleteAction(ColumnFetcher columnFetcher,
            TableChange.DeleteColumn delete)
    {
        Function<List<Field>, List<Field>> listProcessor;
        if (delete.ifExists()) {
            listProcessor = fieldsList -> filterExistingColumns.apply(
                    fieldsList, delete.fieldNames());
        }
        else {
            listProcessor = fieldsList -> filterExistingColumns.andThen(
                    list -> {
                        if (list.size() < delete.fieldNames().length) {
                            throw toRuntime(new VastUserException(
                                    "Some columns do not exist"));
                        }
                        return list;
                    }).apply(fieldsList, delete.fieldNames());
        }
        return (client, tx) -> new DropColumn(columnFetcher
                .andThen(listProcessor)
                .andThen(streamFieldsAsDropColumnContext)
                .apply(client, tx)).accept(client, tx);
    }
            ...
            throws VastUserException
    {
        ...
        VerifyParam.verify(fieldNames.length == 1,
                format("Received multiple column names to add: %s",
                        Arrays.toString(fieldNames)));
        ...
        List<Field> fields = Arrays.stream(add.fieldNames()).map(
                name -> TypeUtil.sparkFieldToArrowField(name, dataType, true,
                        Metadata.empty())).collect(Collectors.toList());
        return (client, tx) -> new AddColumn(
                streamFieldsAsDropColumnContext.apply(fields)).accept(client,
                tx);
    }

    static class CompositeTableChange
            implements VastTableChange
    {
        ...
        @Override
        public void accept(VastClient vastClient,
                VastTransaction vastTransaction)
        {
            changes.forEach(
                    change -> change.accept(vastClient, vastTransaction));
        }
    }

    private class ColumnFetcher
            implements BiFunction<VastClient, VastTransaction, List<Field>>
    {
        ...
        public List<Field> apply(VastClient vastClient, VastTransaction tx)
        {
            if (fields == null) {
                try {
                    String endUser = NDBSparkSessionExtension.getSessionUser(
                            vastConfig);
                    fields = vastClient
