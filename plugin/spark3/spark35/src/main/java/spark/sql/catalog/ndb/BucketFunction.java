            ...
            NamedReference newRef, int numBuckets)
    {
        ...
        if (type instanceof DateType || type instanceof ByteType || type instanceof ShortType || type instanceof IntegerType) {
            transform = l -> (BucketUtil.hash(
                    (Integer) ((Literal) l).value()) & Integer.MAX_VALUE) % numBuckets;
        }
        else if (type instanceof LongType || type instanceof TimestampType || type instanceof TimestampNTZType) {
            transform = l -> (BucketUtil.hash(
                    (Long) ((Literal) l).value()) & Integer.MAX_VALUE) % numBuckets;
        }
        else if (type instanceof DecimalType) {
            transform = l -> (BucketUtil.hash(
                    ((Decimal) ((Literal) l).value()).toJavaBigDecimal()) & Integer.MAX_VALUE) % numBuckets;
        }
        else if (type instanceof StringType) {
            transform = l -> (BucketUtil.hash(
                    ((UTF8String) ((Literal) l).value()).getBytes()) & Integer.MAX_VALUE) % numBuckets;
        }
        else if (type instanceof BinaryType) {
            transform = l -> (BucketUtil.hash(
                    (byte[]) ((Literal) l).value()) & Integer.MAX_VALUE) % numBuckets;
        }
        else {
