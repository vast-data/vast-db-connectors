...
public class VastTableChangeFactory
{
    ...
    private class ColumnFetcher implements BiFunction<VastClient, VastTransaction, List<Field>> {
        ...
        public List<Field> apply(VastClient vastClient, VastTransaction tx)
        {
            if (fields == null) {
                try {
                    fields = vastClient.listColumns(tx, schemaName, tableName, PAGE_SIZE);
                }
        ...
        for(TableChange tableChange: tableChanges) {
            if (tableChange instanceof TableChange.DeleteColumn) {
                ...
                if (shouldSkipDelete(delete)) {
                    LOG.debug("Skipping drop column action for table change: {}", tableChange);
                }
                else {
                    LOG.debug("Adding drop column action for table change: {}", tableChange);
            ...
            throws VastUserException
    {
        ...
        VerifyParam.verify(fieldNames.length == 1, "Received multiple column names for delete");
        String name = fieldNames[0];
         return SPARK_ROW_ID_FIELD.name().equalsIgnoreCase(name);
    }
            ...
            throws VastUserException
    {
        ...
        VerifyParam.verify(oldNames.length == 1, "Received multiple column names for rename");
