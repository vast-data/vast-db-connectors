        ...
        implements Scan, SupportsReportStatistics, SupportsRuntimeV2Filtering
{
    ...
    public Statistics estimateStatistics() {
        ...
        if (scanStatistics == null) {
            final Statistics scanStatistics = estimateBasedOnTableStatistics(this.table.estimateStatistics());
            final double selectivity = FilterEstimator.estimateSelectivity(this.pushDownPredicates, scanStatistics);
            final UnaryOperator<Long> applySelectivity = stat -> (long) (stat * selectivity);
            this.scanStatistics = new TableLevelStatistics(
                    map(scanStatistics.sizeInBytes(), applySelectivity),
                    map(scanStatistics.numRows(), applySelectivity),
                    scanStatistics.columnStats()
            );
            LOG.info("Estimated statistics: selectivity: {}, row count: {} -> {}, size in bytes: {} -> {}", selectivity,
