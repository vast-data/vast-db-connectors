        ...
        implements CatalogExtension
{
    ...
    private SparkConfigurationCheck sparkConfigurationCheck;
    public VastConfig config;
    ...
    public void initialize(String name, CaseInsensitiveStringMap options)
    {
        ...
        try {
            ...
            if (!empty) {
                ...
                sparkConfigurationCheck = new SparkConfigurationCheck(sparkContext.getConf());
            ...
            throws NoSuchTableException
    {
        ...
        try (VastAutocommitTransaction tx = VastAutocommitTransaction.wrap(vastClient, () -> transactionsManager.startTransaction(new StartTransactionContext(false, true)))) {
            ...
            if (vastTableHandleId.isPresent()) {
                ...
                return makeVastTable(
                        schemaName, tableName, vastTableHandleId.get(), schema, () -> this.vastClient, isImport,
        ...
        try (VastAutocommitTransaction tx = VastAutocommitTransaction.wrap(vastClient, () -> transactionsManager.startTransaction(new StartTransactionContext(false, true)))) {
            if (vastClient.schemaExists(tx, schemaName)) {
                if (!vastClient.tableExists(tx, schemaName, tableName)) {
                    ...
                    tx.setCommit(true);
                    return makeVastTable(schemaName, tableName, vastTableHandleId, schema, () -> vastClient, false, true);
                }
                else {
            ...
            throws NoSuchTableException
    {
        ...
        try (VastAutocommitTransaction tx = VastAutocommitTransaction.wrap(vastClient, () -> transactionsManager.startTransaction(new StartTransactionContext(false, true)))) {
            ...
            if (vastTableHandleId.isPresent()) {
                ...
                tx.setCommit(true);
                List<Field> fields = vastClient.listColumns(tx, schemaName, tableName, PAGE_SIZE);
                return makeVastTable(schemaName, tableName, vastTableHandleId.get(), TypeUtil.arrowFieldsListToSparkSchema(fields), () -> vastClient, false, true);
            }
            else {
                throw new NoSuchTableException(ident);
            }
    ...
    {
        return sparkConfigurationCheck.writeError
                .map(error -> (VastTable) new VastTableReadOnly(schemaName, tableName, handleID, schema, clientSupplier, forImportData, isPredicatePushdownEnabled, error))
                .orElseGet(() -> new VastTable(schemaName, tableName, handleID, schema, clientSupplier, forImportData, isPredicatePushdownEnabled));
    }
}
