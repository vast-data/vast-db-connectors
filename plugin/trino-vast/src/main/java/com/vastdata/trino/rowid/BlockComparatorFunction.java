    ...
    public Comparator<Integer> apply(Block rowIdsBlock)
    {
        return switch (rowIdsBlock) {
            case LongArrayBlock longArrayBlock ->
                    Comparator.comparing(longArrayBlock::getLong);
            case Int128ArrayBlock decimalRowIDsBlock ->
                    Comparator.comparing(decimalRowIDsBlock::getInt128);
            case null -> throw new UnsupportedOperationException(
                    "Unsupported block type: null");
            default -> throw new UnsupportedOperationException(
                    "Unsupported block type " + rowIdsBlock
                            .getClass()
                            .getName());
        };
    }
