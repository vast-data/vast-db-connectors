        ...
        implements VastStatisticsStorage<VastTableHandle, TableStatistics>
{
    private static final Logger LOG = Logger.get(
            TrinoPersistentStatistics.class);
    ...
    {
        statisticsUrlHelper = new VastTrinoDependenciesFactory(
                trinoConfig).getStatisticsUrlHelper();
        tag = new VastTrinoDependenciesFactory(
                trinoConfig).getConnectorVersionedStatisticsTag();
        this.client = client;
        this.cache = EvictableCacheBuilder
                .newBuilder()
                .maximumSize(config.getMaxStatisticsFilesSupportedPerSession())
                .build(CacheLoader.from(this::tableStatisticsLoader));
        this.statsSerializer = requireNonNullElseGet(statsSerializer,
                () -> stats ->
                {
                    try {
    ...
    {
        StatisticsUrl extracted = StatisticsUrl.extract(
                tableHandleCacheKey.getTableHandle(), statisticsUrlHelper, tag);
        ...
        {
            try {
                TrinoSerializableTableStatistics serializableTableStatistics = mapper.readValue(
                        bufferBytes, TrinoSerializableTableStatistics.class);
                TableStatistics ts = serializableTableStatistics.getTableStatistics();
                LOG.info("Parsed %s from S3 bucket %s", keyName, bucketName);
                return ts;
            }
            catch (IOException e) {
                LOG.warn(e, "Failed to parse table statistics: %s",
                        bufferBytes);
                return null;
            }
        });
    ...
    public Optional<TableStatistics> getTableStatistics(VastTableHandle table)
    {
        ...
        try {
            LOG.info(
                    "attempting to fetch statistics file for table %s from cache",
                    table.toSchemaTableName());
            Optional<TableStatistics> result = cache.get(cacheKey);
            if (result.isEmpty()) {
                LOG.debug("statistics cache miss for table %s",
                        table.toSchemaTableName());
            }
            else {
                LOG.debug("statistics cache hit for table %s",
                        table.toSchemaTableName());
            }
            return result;
        }
    ...
    @Override
    public void setTableStatistics(VastTableHandle table,
                                   TableStatistics tableStatistics)
    {
        CacheKey cacheKey = new CacheKey(table);
        StatisticsUrl extracted = StatisticsUrl.extract(table,
        ...
        if (!bucketName.equals(BIG_CATALOG_BUCKET_NAME)) {
            ...
            LOG.info("uploading statistics file {} to S3 bucket {}...\n",
                    keyName, bucketName);
            try {
                String tableStatisticsStr = statsSerializer.apply(
                        tableStatistics);
                ...
                LOG.warn(
                        "failed to upload table statistics file to S3 bucket %s",
                        bucketName);
                throw e;
            }
        }
    ...
    public void deleteTableStatistics(VastTableHandle table)
    {
        ...
        LOG.info("invalidating statistics for table %s from cache",
                tableHandleCacheKey.getTableHandle());
    ...
    public static class CacheKey
    {
        ...
        public boolean equals(Object obj)
        {
            ...
            return Objects.equals(this.tableHandle.getHandleID().getHandle(),
                    other.tableHandle.getHandleID().getHandle());
        }

        @Override
        public int hashCode()
        {
            return Objects.hash(tableHandle.getHandleID().getHandle());
        }
    }
}
