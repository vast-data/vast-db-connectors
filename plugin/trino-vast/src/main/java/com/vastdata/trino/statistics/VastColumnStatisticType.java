...
{
    MIN_VALUE(new FunctionName("min")), MAX_VALUE(
        new FunctionName("max")), NUMBER_OF_DISTINCT_VALUES(
        new FunctionName("approx_distinct")), NUMBER_OF_NON_NULL_VALUES(
        new FunctionName("count")), TOTAL_SIZE_IN_BYTES(
        ColumnStatisticType.TOTAL_SIZE_IN_BYTES),
    /**/;
    ...
    {
        return VastColumnStatisticType.valueOf(
                columnStatisticMetadata.getConnectorAggregationId());
    }
    ...
    public ColumnStatisticMetadata createColumnStatisticMetadata(String columnName)
    {
        ...
        if (columnStatisticType.isPresent()) {
            return new ColumnStatisticMetadata(columnName,
                    connectorAggregationId, columnStatisticType.orElseThrow());
        }
        return new ColumnStatisticMetadata(columnName, connectorAggregationId,
                aggregationName.orElseThrow());
    }
}
