        ...
        implements SystemAccessControl
{
    ...
    private static boolean enableRowColumnSecurity = false;
    private static boolean enableEndUserImpersonation = false;

    private final VastClient client;
    private final VastTrinoTransactionHandleManager transactionManager;

    protected static void setup(final boolean shouldEnableRowColumnSecurity, final boolean shouldEnableEndUserImpersonation)
    {
        LOG.debug("Setting %s.enable: %s->%s, shouldEnableRowColumnSecurity=%s, shouldEnableEndUserImpersonation=%s",
                VastSecurityAccessControl.class.getSimpleName(), enable, true,
                shouldEnableRowColumnSecurity, shouldEnableEndUserImpersonation);
        enable = true;
                                                           ...
                                                           final String endUser)
    {
        if (!isRowColumnSecurityEnabled()) {
            final String message = String.format(
                    ...
                    enable, enableRowColumnSecurity, enableEndUserImpersonation);
        ...
        try (final VastAutocommitTransaction tx = VastAutocommitTransaction.createNewOrReuseFromEnv(client, () -> transactionManager.startTransaction(endUser), endUser)) {
            ...
            if (isImportDataTableName(tableName)) {
                ...
                LOG.debug("getRowColumnSecurity - stripping import suffix for table name security policy fetching", tableName);
            }
            return client.getRowColumnSecurity(tx, schema, tableName, isEndUserImpersonationEnabled() ? endUser : null);
        }
    ...
    {
        if (isRowColumnSecurityEnabled()) {
            return CATALOG_NAME.equals(table.getCatalogName())
                    && !INFORMATION_SCHEMA.equals(table.getSchemaTableName().getSchemaName());
        }
        else {
    ...
    public List<ViewExpression> getRowFilters(final SystemSecurityContext context, final CatalogSchemaTableName tableName)
    {
        ...
        if (isRelevant(tableName)) {
            ...
            LOG.debug("VastSecurityAccessControl#getRowFilters endUser=%s NOT ignoring policy=%s", endUser, policy);
            final List<ViewExpression> result = policy.getRowFilters().stream()
                    .map(filter ->
                            ViewExpression.builder()
                                    ...
                                    .build())
                    .toList();
            LOG.debug("VastSecurityAccessControl#getRowFilters endUser=%s result=%s", endUser, result);
            return result;
        }
        else {
            LOG.debug("getRowFilters endUser=%s ignores request because it's irrelevant: table=%s", endUser, tableName);
            return List.of();
        }
    }
    ...
    public void checkCanSelectFromColumns(final SystemSecurityContext context, final CatalogSchemaTableName table, final Set<String> columns)
    {
        ...
        if (isRelevant(table)) {
            ...
            final String schema = schemaTableName.getSchemaName();
            final RowColumnSecurityResponse policy = getRowColumnSecurity(schema, schemaTableName, endUser);
            LOG.debug("VastSecurityAccessControl#checkCanSelectFromColumns endUser=%s, policy=%s", endUser, policy);
            if (policy.getDeniedColumns().stream().anyMatch(columns::contains)) {
                LOG.debug("VastSecurityAccessControl#checkCanSelectFromColumns endUser=%s requested columns contain at least one denied column: requested columns: %s, denied columns: %s",
            ...
            if (!policy.getAllowedColumns().isEmpty()) {
                if (!(columns.stream().allMatch(column -> policy.getAllowedColumns().contains(column) || policy.getMaskedColumns().containsKey(column)))) {
                    LOG.debug("VastSecurityAccessControl#checkCanSelectFromColumns endUser=%s at least one requested columns is not (allowed or masked): "
            ...
            LOG.debug("checkCanSelectFromColumns endUser=%s ignores request because it's irrelevant: table=%s", endUser, table);
        }
    }
    ...
    public void checkCanCreateViewWithSelectFromColumns(final SystemSecurityContext context, final CatalogSchemaTableName table, final Set<String> columns)
    {
        ...
        if (isRelevant(table)) {
            ...
            final String schema = schemaTableName.getSchemaName();
            final RowColumnSecurityResponse policy = getRowColumnSecurity(schema, schemaTableName, endUser);
            LOG.debug("VastSecurityAccessControl#checkCanCreateViewWithSelectFromColumns endUser=%s policy=%s", endUser, policy);
            if (policy.getDeniedColumns().stream().anyMatch(columns::contains)) {
                LOG.debug("VastSecurityAccessControl#checkCanCreateViewWithSelectFromColumns endUser=%s requested columns contain at least one denied column: requested columns: %s, denied columns: %s", endUser, columns, policy.getDeniedColumns());
                AccessDeniedException.denyCreateViewWithSelect(table.toString(), context.getIdentity());
            }
            if (!policy.getAllowedColumns().isEmpty()) {
                if (!(columns.stream().allMatch(column -> policy.getAllowedColumns().contains(column) || policy.getMaskedColumns().containsKey(column)))) {
                    LOG.debug("VastSecurityAccessControl#checkCanCreateViewWithSelectFromColumns endUser=%s at least one requested columns is not (allowed or masked): requested columns: %s, allowed columns: %s, masked columns: %s", endUser, columns, policy.getAllowedColumns(), policy.getMaskedColumns());
                    AccessDeniedException.denyCreateViewWithSelect(table.toString(), context.getIdentity());
                }
            }
        }
        else {
            LOG.debug("checkCanCreateViewWithSelectFromColumns endUser=%s ignores request because it's irrelevant: table=%s", endUser, table);
        }
    }
    ...
    public Map<ColumnSchema, ViewExpression> getColumnMasks(final SystemSecurityContext context, final CatalogSchemaTableName tableName, final List<ColumnSchema> columns)
    {
        ...
        if (isRelevant(tableName)) {
            ...
            final String schema = schemaTableName.getSchemaName();
            final RowColumnSecurityResponse policy = getRowColumnSecurity(schema, schemaTableName, endUser);
            LOG.debug("VastSecurityAccessControl#getColumnMasks endUser=%s policy=%s", endUser, policy);
            ...
            LOG.debug("getColumnMasks endUser=%s ignores request because it's irrelevant: tableName=%s", endUser, tableName);
    ...
    @Override
    public void checkCanSetEntityAuthorization(final SystemSecurityContext context, final EntityKindAndName entityKindAndName, final TrinoPrincipal principal)
    {
        SystemAccessControl.super.checkCanSetEntityAuthorization(context, entityKindAndName, principal);
    }
