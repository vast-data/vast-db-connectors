...
public class VastProjectionPushdown
{
    ...
    private static final Set<FunctionName> pushThroughFunctions = Set.of(
            AND_FUNCTION_NAME, OR_FUNCTION_NAME);

    // Will be kept unmodified and evaluated in Trino, to allow other parts of the expression to be pushed down into VAST
    private static final Set<FunctionName> functionsToKeep = Set.of(
    ...
    static Optional<Variable> matchUnaryCalls(ConnectorExpression expression,
                                              FunctionName... names)
    {
        for (FunctionName name : names) {
            ...
            expression = call.getArguments().getFirst();
        }
    ...
    public static void forVariableChildren(ConnectorExpression expression,
                                           Consumer<Variable> consumer)
    {
        ...
        expression
                .getChildren()
                .forEach(child -> forVariableChildren(child, consumer));
    }

    public Optional<Result> apply(ConnectorExpression expression)
    {
        LOG.debug("trying to pushdown %s", expression);
        Optional<Variable> result = matchUnaryCalls(expression,
                IS_NULL_FUNCTION_NAME);
        if (result.isPresent()) {
            Variable variable = result.orElseThrow();
            return Optional.of(
                    Result.from(VastExpression.from(IS_NULL, variable)));
        }
        result = matchUnaryCalls(expression, NOT_FUNCTION_NAME,
                IS_NULL_FUNCTION_NAME);
        if (result.isPresent()) {
            Variable variable = result.orElseThrow();
            return Optional.of(
                    Result.from(VastExpression.from(IS_VALID, variable)));
        }
    ...
    public Optional<Result> matchComparison(ConnectorExpression expression)
    {
        ...
        if (pushThroughFunctions.contains(call.getFunctionName())) {
            ...
            return Optional.of(new Result(
                    new Call(call.getType(), call.getFunctionName(), args),
                    pushedDown));
        }

        ArrowComputeFunction function = supportedComparisons.get(
        ...
        if (nonNull(function)) {
            ...
            return Optional.of(Result.from(
                    VastExpression.from(function, variable, List.of(constant),
                            session)));
        }
        ...
        if (functionsToKeep.contains(call.getFunctionName())) {
            ...
            List<VastExpression> variables = builder
                    .build()
                    .stream()
                    .map(VastExpression::identity)
                    .collect(Collectors.toList());
    ...
    public static class Result
    {
        ...
        private Result(ConnectorExpression remaining,
                       List<VastExpression> pushedDown)
        {
        ...
        {
            Variable newVariable = new Variable(expression.toString(),
                    expression.getVariableType());
