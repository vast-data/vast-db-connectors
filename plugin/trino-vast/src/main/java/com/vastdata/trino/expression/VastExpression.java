...
public class VastExpression
{
    ...
    @JsonCreator
    public VastExpression(@JsonProperty("function") ArrowComputeFunction function,
    ...
    public static VastExpression from(ArrowComputeFunction function,
                                      Variable variable)
    {
                                      ...
                                      ConnectorSession session)
    {
        ...
        List<Block> blocks = args
                .stream()
                .map(constant -> nativeValueToBlock(constant.getType(),
                        constant.getValue()))
        ...
        if (nonNull(function)) {
            ...
            for (Block block : blocks) {
                builder
                        .append(',')
                        .append(variable.getType().getObjectValue(block, 0));
            }
            builder.append(')');
        }
        else {
        ...
        return new VastExpression(function, name, type, blocks,
                builder.toString());
    }
