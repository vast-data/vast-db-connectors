        ...
        extends TrinoExpressionSerializer
{
    private static final Logger LOG = Logger.get(
            TrinoPredicateSerializer.class);
    ...
    public TrinoPredicateSerializer(ShapingLoggerFactory shapingLoggerFactory,
                                    ...
                                    EnumeratedSchema enumeratedSchema)
    {
        ...
        shapingLogger.debug(
                "serializing %s, %s and %s (schema=%s) to Arrow Compute IR flatbuffer format",
                tupleDomain, complexPredicate, substringMatches,
                enumeratedSchema);
        this.complexPredicate = complexPredicate;
        this.enumeratedSchema = enumeratedSchema;

        this.domainsMap = tupleDomain
                .getDomains()
                .orElseThrow()
                .entrySet()
                .stream()
                .collect(Collectors.toMap(
                        entry -> getColumnPosition(entry.getKey(),
                                enumeratedSchema), Map.Entry::getValue));
        shapingLogger.debug("domainsMap=%s", domainsMap);
        ...
        for (VastSubstringMatch substringMatch : substringMatches) {
            long columnId = getColumnPosition(substringMatch.column(),
                    enumeratedSchema).longValue();
            String pattern = substringMatch.pattern();
            this.substringsMap.put(columnId, pattern);
        }
        shapingLogger.debug("substringsMap=%s", substringsMap);
    }

    private static Long getColumnPosition(VastColumnHandle column,
                                          EnumeratedSchema enumeratedSchema)
    {
        ImmutableList.Builder<Integer> builder = ImmutableList.builder();
        enumeratedSchema.collectProjectionIndices(
                column.getBaseField().getName(), column.getProjectionPath(),
                builder::add);
        return Iterables
                .getOnlyElement(builder.build())
                .longValue(); // only predicates over leaf columns are supported
    }
    ...
    public int serialize()
    {
        if (complexPredicate.isPresent()) {
            ...
            shapingLogger.debug("serializing complex predicate: %s",
                    complexPredicate);
        ...
        for (Map.Entry<Long, Domain> entry : domainsMap.entrySet()) {
            shapingLogger.debug("serializing %s for column position %s",
                    entry.getValue(), entry.getKey());
            offsets[i++] = buildDomain(buildColumn(entry.getKey()),
                    entry.getValue());
        }
        for (Map.Entry<Long, String> entry : substringsMap.entrySet()) {
            shapingLogger.debug(
                    "serializing substring '%s' match for column position %s",
                    entry.getValue(), entry.getKey());
            int column = buildColumn(entry.getKey());
            int literal = buildLiteral(VARCHAR,
                    Slices.utf8Slice(entry.getValue()));
    ...
    private int buildPredicate(ComplexPredicate predicate)
    {
        if (predicate instanceof LogicalFunction func) {
            IntStream args = func
                    .children()
                    .stream()
                    .mapToInt(child -> buildPredicate(child));
            if (func.name().equals(AND_FUNCTION_NAME.getName())) {
            ...
            throw new UnsupportedOperationException(
                    format("Unsupported function: %s", func));
        }
        if (predicate instanceof ColumnDomain columnDomain) {
            long position = getColumnPosition(columnDomain.getColumn(),
                    enumeratedSchema);
            return buildDomain(buildColumn(position), columnDomain.getDomain());
        }
        throw new UnsupportedOperationException(
                format("Unsupported predicate: %s", predicate));
    }
    ...
    private int buildDomain(int column, Domain domain)
    {
        ...
        if (values.isAll()) {
            ...
            throw new IllegalArgumentException(
                    format("redundant domain %s for column %d", domain,
                            column));
        }
        ...
        verify(i == length, "%d predicates were set (instead of %d)", i,
                length);
    ...
    private int buildRange(int column, Range range)
    {
        if (range.isSingleValue()) {
            return buildEqual(column,
                    buildLiteral(range.getType(), range.getSingleValue()));
        }
        verify(!range.isAll(), "redundant %s range for column %d", range,
                column);
        if (range.isHighUnbounded()) {
            int literal = buildLiteral(range.getType(),
        ...
        int lowLiteral = buildLiteral(range.getType(),
                range.getLowBoundedValue());
        int highLiteral = buildLiteral(range.getType(),
                range.getHighBoundedValue());
        return buildAnd(
