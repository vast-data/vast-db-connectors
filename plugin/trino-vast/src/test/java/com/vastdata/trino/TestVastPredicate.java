...
public class TestVastPredicate
{
    ...
    public void testAll()
    {
        ...
        TrinoPredicateSerializer builder = new TrinoPredicateSerializer(
                new ShapingLoggerFactory(new VastConfig()), TupleDomain.all(),
                Optional.empty(), List.of(), enumeratedSchema);
        ...
        assertThat(flatbuffer.sizedByteArray()).asHexString().isEqualTo(
                "0C00000008000C000700080008000000000000030C00000008000C00080004000800000008000000080000000000000003000000616E6400");
    ...
    public void testRelation()
    {
        ...
        byte[] bytes = HexFormat.of().parseHex(
                "");
        Relation relation = Relation.getRootAsRelation(ByteBuffer.wrap(bytes));
        Source source = (Source) relation.impl(new Source());
        assertThat(source.name()).isEqualTo(
                "/bucket-for-filter/schema-for-filter/table1");

        FieldIndex.Vector projections = source.projectionVector();
        assertThat(projections.length()).isEqualTo(1);
        assertThat(projections.get(0).position()).isEqualTo(
                0); // project field #0 in schema
        ...
        Field expected = Field.nullablePrimitive(null,
